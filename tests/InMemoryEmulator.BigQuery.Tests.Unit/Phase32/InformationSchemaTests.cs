...
public class InformationSchemaTests
{
	...
	#region ROUTINE_OPTIONS view

	// Ref: https://cloud.google.com/bigquery/docs/information-schema-routine-options
	//   "The INFORMATION_SCHEMA.ROUTINE_OPTIONS view contains one row for each
	//    option, for each routine in a dataset."

	[Fact]
	public void RoutineOptions_ReturnsDescriptionOption()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		var routine = new InMemoryRoutine(
			"test_ds", "my_fn", "SCALAR_FUNCTION", "SQL", "x + 1",
			[("x", "INT64")], returnType: "INT64");
		routine.Description = "My function";
		ds.Routines["my_fn"] = routine;
		...
		Assert.Single(rows);
		Assert.Equal("my_fn", rows[0].F[0].V?.ToString());
		Assert.Equal("description", rows[0].F[1].V?.ToString());
		Assert.Equal("STRING", rows[0].F[2].V?.ToString());
		Assert.Contains("My function", rows[0].F[3].V?.ToString());
	}

	[Fact]
	public void RoutineOptions_NoDescriptions_ReturnsEmpty()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		ds.Routines["fn_no_desc"] = new InMemoryRoutine(
			"test_ds", "fn_no_desc", "SCALAR_FUNCTION", "SQL", "x + 1",
			[("x", "INT64")], returnType: "INT64");

		var (_, rows) = exec.Execute(
			"SELECT option_name FROM INFORMATION_SCHEMA.ROUTINE_OPTIONS");
	...
	//   "The INFORMATION_SCHEMA.PARAMETERS view contains one row for each parameter
	//    of a routine."

	[Fact]
	public void Parameters_ReturnsReturnTypeAndParams()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		ds.Routines["add_fn"] = new InMemoryRoutine(
			"test_ds", "add_fn", "SCALAR_FUNCTION", "SQL", "a + b",
			[("a", "INT64"), ("b", "INT64")], returnType: "FLOAT64");
		...
		Assert.Equal("add_fn", rows[0].F[0].V?.ToString());
		Assert.Equal("0", rows[0].F[1].V?.ToString());
		Assert.Null(rows[0].F[2].V);
		Assert.Null(rows[0].F[3].V);
		Assert.Equal("FLOAT64", rows[0].F[4].V?.ToString());

		// ordinal_position=1 is parameter "a"
		Assert.Equal("1", rows[1].F[1].V?.ToString());
		Assert.Equal("IN", rows[1].F[2].V?.ToString());
		Assert.Equal("a", rows[1].F[3].V?.ToString());
		Assert.Equal("INT64", rows[1].F[4].V?.ToString());

		// ordinal_position=2 is parameter "b"
		Assert.Equal("2", rows[2].F[1].V?.ToString());
		Assert.Equal("IN", rows[2].F[2].V?.ToString());
		Assert.Equal("b", rows[2].F[3].V?.ToString());
		Assert.Equal("INT64", rows[2].F[4].V?.ToString());
	}

	[Fact]
	public void Parameters_NoParams_ReturnsOnlyReturnType()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		ds.Routines["const_fn"] = new InMemoryRoutine(
			"test_ds", "const_fn", "SCALAR_FUNCTION", "SQL", "42",
			[], returnType: "INT64");

		var (_, rows) = exec.Execute(
			"SELECT ordinal_position, parameter_mode, data_type " +
			"FROM INFORMATION_SCHEMA.PARAMETERS");

		Assert.Single(rows);
		Assert.Equal("0", rows[0].F[0].V?.ToString());
		Assert.Null(rows[0].F[1].V);
		Assert.Equal("INT64", rows[0].F[2].V?.ToString());
	}

	[Fact]
	public void Parameters_EmptyDataset_ReturnsNoRows()
	{
		var (exec, _) = CreateExecutor();
	...
	#region SEARCH_INDEXES view

	// Ref: https://cloud.google.com/bigquery/docs/information-schema-indexes
	//   "The INFORMATION_SCHEMA.SEARCH_INDEXES view has one row for each search index."

	[Fact]
	public void SearchIndexes_CreateIndex_ReturnsIndexInfo()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		AddTable(ds, "my_table", SimpleSchema);

		exec.Execute("CREATE SEARCH INDEX my_idx ON my_table (id)");

		var (_, rows) = exec.Execute(
			"SELECT index_name, table_name, index_status, analyzer " +
			"FROM INFORMATION_SCHEMA.SEARCH_INDEXES");

		Assert.Single(rows);
		Assert.Equal("my_idx", rows[0].F[0].V?.ToString());
		Assert.Equal("my_table", rows[0].F[1].V?.ToString());
		Assert.Equal("ACTIVE", rows[0].F[2].V?.ToString());
		Assert.Equal("LOG_ANALYZER", rows[0].F[3].V?.ToString());
	}

	[Fact]
	public void SearchIndexes_CreateWithNoOpAnalyzer_ReturnsCustomAnalyzer()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		AddTable(ds, "my_table", SimpleSchema);
		...
		exec.Execute(
			"CREATE SEARCH INDEX my_idx ON my_table (id) OPTIONS(analyzer='NO_OP_ANALYZER')");

		var (_, rows) = exec.Execute(
			"SELECT index_name, analyzer FROM INFORMATION_SCHEMA.SEARCH_INDEXES");

		Assert.Single(rows);
		Assert.Equal("my_idx", rows[0].F[0].V?.ToString());
		Assert.Equal("NO_OP_ANALYZER", rows[0].F[1].V?.ToString());
	}

	[Fact]
	public void SearchIndexes_DropIndex_RemovesFromView()
	{
		var (exec, store) = CreateExecutor();
		var ds = store.Datasets["test_ds"];
		AddTable(ds, "my_table", SimpleSchema);
		...
		exec.Execute("CREATE SEARCH INDEX my_idx ON my_table (id)");
		exec.Execute("DROP SEARCH INDEX my_idx ON my_table");

		var (_, rows) = exec.Execute(
			"SELECT index_name FROM INFORMATION_SCHEMA.SEARCH_INDEXES");

		Assert.Empty(rows);
	}

	[Fact]
	public void SearchIndexes_EmptyDataset_ReturnsNoRows()
	{
		var (exec, _) = CreateExecutor();
	...
	//   "The INFORMATION_SCHEMA.JOBS view contains metadata about BigQuery jobs."
	//   JOBS is project-scoped, not dataset-scoped.

	[Fact]
	public void Jobs_ReturnsJobInfo()
	{
		var (exec, store) = CreateExecutor();
		store.Jobs["j1"] = new InMemoryJob("test-project", "j1")
		{
			Query = "SELECT 1",
		...
		var (_, rows) = exec.Execute(
			"SELECT job_id, project_id, job_type, statement_type, state " +
			"FROM INFORMATION_SCHEMA.JOBS");

		Assert.Single(rows);
		Assert.Equal("j1", rows[0].F[0].V?.ToString());
		Assert.Equal("test-project", rows[0].F[1].V?.ToString());
		Assert.Equal("QUERY", rows[0].F[2].V?.ToString());
		Assert.Equal("SELECT", rows[0].F[3].V?.ToString());
		Assert.Equal("DONE", rows[0].F[4].V?.ToString());
	}

	[Fact]
	public void Jobs_EmptyStore_ReturnsNoRows()
	{
		var (exec, _) = CreateExecutor();

		var (_, rows) = exec.Execute(
			"SELECT job_id FROM INFORMATION_SCHEMA.JOBS");
