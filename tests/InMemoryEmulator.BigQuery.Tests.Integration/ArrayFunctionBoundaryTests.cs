...
public class ArrayFunctionBoundaryTests : IAsyncLifetime
{
	...
	// ---- ARRAY_REVERSE ----
	[Fact] public async Task ArrayReverse_Ints() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_REVERSE([1, 2, 3])) AS x), ',')"); Assert.Equal("3,2,1", v); }
	...
	[Fact] public async Task ArrayReverse_Single() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_REVERSE([42])) AS x), ',')"); Assert.Equal("42", v); }
	[Fact] public async Task ArrayReverse_Empty() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_REVERSE(CAST([] AS ARRAY<INT64>))) AS x), ',')"); Assert.Equal("", v); }

	// ---- ARRAY_CONCAT ----
	[Fact] public async Task ArrayConcat_TwoArrays() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_CONCAT([1, 2], [3, 4])) AS x), ',')"); Assert.Equal("1,2,3,4", v); }
	[Fact] public async Task ArrayConcat_ThreeArrays() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_CONCAT([1], [2], [3])) AS x), ',')"); Assert.Equal("1,2,3", v); }
	[Fact] public async Task ArrayConcat_EmptyFirst() { var v = await Scalar("SELECT ARRAY_TO_STRING(ARRAY(SELECT CAST(x AS STRING) FROM UNNEST(ARRAY_CONCAT(CAST([] AS ARRAY<INT64>), [1, 2])) AS x), ',')"); Assert.Equal("1,2", v); }
