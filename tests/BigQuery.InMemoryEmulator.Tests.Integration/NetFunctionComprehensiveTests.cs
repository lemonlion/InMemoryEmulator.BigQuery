...
public class NetFunctionComprehensiveTests : IAsyncLifetime
{
	...
	[Fact] public async Task NetHost_NoScheme() => Assert.Equal("example.com", await Scalar("SELECT NET.HOST('//example.com/path')"));
	...
	[Fact] public async Task NetPublicSuffix_CoUk() => Assert.Equal("co.uk", await Scalar("SELECT NET.PUBLIC_SUFFIX('www.example.co.uk')"));
	...
	[Fact] public async Task NetRegDomain_CoUk() => Assert.Equal("example.co.uk", await Scalar("SELECT NET.REG_DOMAIN('www.example.co.uk')"));
	...
	// ---- NET.IP_TRUNC ----
	[Fact] public async Task NetIpTrunc_IPv4_24() => Assert.Equal("192.168.1.0", await Scalar("SELECT NET.IP_TO_STRING(NET.IP_TRUNC(NET.IP_FROM_STRING('192.168.1.100'), 24))"));
	[Fact] public async Task NetIpTrunc_IPv4_16() => Assert.Equal("192.168.0.0", await Scalar("SELECT NET.IP_TO_STRING(NET.IP_TRUNC(NET.IP_FROM_STRING('192.168.1.100'), 16))"));

	// ---- NET.IP_NET_MASK ----
	[Fact] public async Task NetIpNetMask_IPv4_24() => Assert.Equal("255.255.255.0", await Scalar("SELECT NET.IP_TO_STRING(NET.IP_NET_MASK(4, 24))"));
	[Fact] public async Task NetIpNetMask_IPv4_16() => Assert.Equal("255.255.0.0", await Scalar("SELECT NET.IP_TO_STRING(NET.IP_NET_MASK(4, 16))"));
