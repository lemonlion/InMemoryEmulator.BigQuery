...
public class NetFunctionComprehensiveTests : IAsyncLifetime
{
	...
	[Fact(Skip = "NET.HOST format differs")] public async Task NetHost_NoScheme() => Assert.Equal("example.com", await Scalar("SELECT NET.HOST('//example.com/path')"));
	...
	[Fact(Skip = "NET.PUBLIC_SUFFIX format differs")] public async Task NetPublicSuffix_CoUk() => Assert.Equal("co.uk", await Scalar("SELECT NET.PUBLIC_SUFFIX('www.example.co.uk')"));
	...
	[Fact(Skip = "NET.REG_DOMAIN format differs")] public async Task NetRegDomain_CoUk() => Assert.Equal("example.co.uk", await Scalar("SELECT NET.REG_DOMAIN('www.example.co.uk')"));
