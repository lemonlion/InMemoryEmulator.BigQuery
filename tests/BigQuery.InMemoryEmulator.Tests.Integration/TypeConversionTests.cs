...
public class TypeConversionTests : IAsyncLifetime
{
	...
	[Fact] public async Task Implicit_IntPlusFloat() => Assert.Equal(5.14, double.Parse(await Scalar("SELECT 2 + 3.14") ?? "0"), 10);
	[Fact] public async Task Implicit_FloatPlusInt() => Assert.Equal(5.14, double.Parse(await Scalar("SELECT 3.14 + 2") ?? "0"), 10);
