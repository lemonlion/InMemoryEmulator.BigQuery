...
public class AggregateFunctionComprehensiveTests : IAsyncLifetime
{
	...
	// ---- ARRAY_CONCAT_AGG ----
	[Fact(Skip = "ARRAY_CONCAT_AGG doubles results from subquery")]
	...
	[Fact] public async Task Corr_Perfect() { var v = double.Parse(await Scalar("SELECT CORR(x, x) FROM (SELECT 1 AS x UNION ALL SELECT 2 UNION ALL SELECT 3)") ?? "0"); Assert.Equal(1.0, v, 10); }
