...
public class StringPatternMatchTests : IAsyncLifetime
{
	...
	// ---- REGEXP_CONTAINS ----
	[Fact] public async Task RegexpContains_Basic() => Assert.Equal("True", await Scalar("SELECT REGEXP_CONTAINS('hello123', '\\\\d+')"));
	...
	// ---- REGEXP_EXTRACT ----
	[Fact] public async Task RegexpExtract_Digits() => Assert.Equal("123", await Scalar("SELECT REGEXP_EXTRACT('hello123world', '\\\\d+')"));
	...
	// ---- REGEXP_REPLACE ----
	[Fact] public async Task RegexpReplace_Digits() => Assert.Equal("helloXworld", await Scalar("SELECT REGEXP_REPLACE('hello123world', '\\\\d+', 'X')"));
	...
	// ---- REGEXP_EXTRACT_ALL ----
	[Fact] public async Task RegexpExtractAll_Digits()
	{
