...
public class AggregateFunctionBoundaryTests : IAsyncLifetime
{
	...
	[Fact(Skip = "Needs investigation")] public async Task Count_Empty() => Assert.Equal("0", await Scalar("SELECT COUNT(*) FROM UNNEST(CAST([] AS ARRAY<INT64>)) AS x"));
	...
	[Fact(Skip = "Needs investigation")] public async Task SumDistinct() => Assert.Equal("6", await Scalar("SELECT SUM(DISTINCT x) FROM UNNEST([1,1,2,2,3]) AS x"));
	[Fact(Skip = "Needs investigation")] public async Task AvgDistinct() { var v = double.Parse(await Scalar("SELECT AVG(DISTINCT x) FROM UNNEST([1,1,2,2,3]) AS x") ?? "0"); Assert.Equal(2.0, v); }
}
